...
instead of WebSearch for SERP data. Run `uv run "${CLAUDE_PLUGIN_ROOT:-$HOME/.claude/skills/seo}/scripts/dataforseo_costs.py" check serp_organic_live_advanced --count N`
...
- All URLs fetched via `uv run "${CLAUDE_PLUGIN_ROOT:-$HOME/.claude/skills/seo}/scripts/fetch_page.py"` (SSRF protection via `validate_url()`)
