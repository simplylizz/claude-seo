...
instead of WebSearch for SERP data. Run `uv run scripts/dataforseo_costs.py check serp_organic_live_advanced --count N`
...
- All URLs fetched via `uv run scripts/fetch_page.py` (SSRF protection via `validate_url()`)
