...
- Vulnerabilities in upstream dependencies — please report those to their respective maintainers. We track CVE-driven version floors in each script's PEP 723 inline metadata and in `pyproject.toml`.
...
- Python scripts declare dependencies via PEP 723 inline metadata; `uv run` resolves them per-script into an isolated, cached environment — no global installs.
