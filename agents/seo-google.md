...
1. Check credentials: `uv run "${CLAUDE_PLUGIN_ROOT:-$HOME/.claude/skills/seo}/scripts/google_auth.py" --check --json`
...
### Tier 0 (API Key Only)
- Run PSI + CrUX on homepage: `uv run "${CLAUDE_PLUGIN_ROOT:-$HOME/.claude/skills/seo}/scripts/pagespeed_check.py" <url> --json`
- Run CrUX History for origin: `uv run "${CLAUDE_PLUGIN_ROOT:-$HOME/.claude/skills/seo}/scripts/crux_history.py" <origin> --origin --json`
- Report CWV field data with traffic-light ratings

### Tier 1 (+ Service Account)
- All Tier 0 checks
- GSC top queries/pages (28 days): `uv run "${CLAUDE_PLUGIN_ROOT:-$HOME/.claude/skills/seo}/scripts/gsc_query.py" --property <prop> --json`
- URL Inspection on homepage + key pages: `uv run "${CLAUDE_PLUGIN_ROOT:-$HOME/.claude/skills/seo}/scripts/gsc_inspect.py" <url> --json`
- GSC sitemap status: `uv run "${CLAUDE_PLUGIN_ROOT:-$HOME/.claude/skills/seo}/scripts/gsc_query.py" sitemaps --property <prop> --json`

### Tier 2 (Full)
- All Tier 1 checks
- GA4 organic traffic (28 days): `uv run "${CLAUDE_PLUGIN_ROOT:-$HOME/.claude/skills/seo}/scripts/ga4_report.py" --property <id> --json`
- Top organic landing pages: `uv run "${CLAUDE_PLUGIN_ROOT:-$HOME/.claude/skills/seo}/scripts/ga4_report.py" --property <id> --report top-pages --json`
...
```bash
uv run "${CLAUDE_PLUGIN_ROOT:-$HOME/.claude/skills/seo}/scripts/google_report.py" --type full --data data.json --domain DOMAIN --format pdf --json
```
