...
| New | What it does |
|---|---|
| `uv run scripts/content_quality.py` | QRG-aligned filler / AI-pattern / information-density scorer |
| `uv run scripts/content_humanize.py` | 40+ deterministic AI-phrasing replacements |
| `uv run scripts/content_verify.py` | Claim extraction + citation-gap detection |
| `uv run scripts/domain_history.py` | WHOIS-driven expired-domain abuse check |
| `uv run scripts/seo_updates.py` | Primary-source Google updates changelog |
...
| New | What it does |
|---|---|
| `uv run scripts/preload_check.py` | Speculation Rules + bfcache + prerender + LCP preload audit |
| `uv run scripts/indexnow_submit.py` | Submit up to 10k URLs to IndexNow (Bing/Yandex/Seznam/Naver) |
| `uv run scripts/lcp_subparts.py` | LCP decomposition via CrUX (TTFB, load delay, load duration, render delay) |
| `uv run scripts/unlighthouse_run.py` | Multi-page Lighthouse via the MIT Unlighthouse CLI |

### Schema completeness (Phase D)

| New | What it does |
|---|---|
| `uv run scripts/schema_generate.py reservation/order/discussion/profile` | JSON-LD generators for the four high-leverage v2 types |
| `uv run scripts/schema_ecommerce_validate.py` | Product schema policy validator (hasMerchantReturnPolicy, shippingDetails, MemberProgram, EU energy class, ProductGroup) |
...
| New | What it does |
|---|---|
| `uv run scripts/parasite_risk.py` | Site-reputation-abuse risk scanner per Nov 2024 Google policy |
...
| New | What it does |
|---|---|
| `uv run scripts/gbp_deprecation_lint.py` | Detects retired GBP chat / `.business.site` / Q&A references |
...
| `uv run scripts/portability_check.py` | Cross-platform SKILL.md frontmatter lint |
...
| New | What it does |
|---|---|
| `uv run scripts/release_sign.py` | Generate a SHA-256 manifest of every git-tracked file |
| `uv run scripts/verify_release.py` | Verify a checkout against a signed manifest |
...
# Scripts auto-install their deps on first run via PEP 723 inline metadata —
# there is no requirements.txt. Run any script with: uv run scripts/<name>.py

# Confirm Playwright Chromium is installed (used by render_page)
uv run --with playwright python -m playwright install chromium

# Verify manifest consistency
uv run --group dev pytest tests/test_manifest_consistency.py -v
```
