...
|---|---|
| `/seo bing links <url>` | `uv run scripts/bing_webmaster.py links <url>` |
| `/seo bing compare <urlA> <urlB>` | `uv run scripts/bing_webmaster.py compare <urlA> <urlB>` |
| `/seo bing submit <url>` (single URL) | `uv run scripts/indexnow_submit.py --host ... --urls <url>` |
| `/seo bing submit-batch <file>` | `uv run scripts/indexnow_submit.py --urls-file <file>` |
| `/seo bing verify-indexnow` | `uv run scripts/indexnow_submit.py --verify-only` |
