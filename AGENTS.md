...
```bash
uv run scripts/portability_check.py
```
...
uv run scripts/fetch_page.py https://example.com

# HTML parsing for SEO elements
uv run scripts/parse_html.py https://example.com

# PageSpeed Insights
uv run scripts/pagespeed_check.py https://example.com --json

# Drift baseline
uv run scripts/drift_baseline.py https://example.com

# DataForSEO (requires credentials)
DATAFORSEO_USERNAME=user DATAFORSEO_PASSWORD=pass uv run scripts/dataforseo_merchant.py search "keyword"
```
...
- Scripts run via `uv run scripts/<name>.py` (PEP 723 inline deps; no venv to activate). If `uv` is not on PATH, install it with `curl -LsSf https://astral.sh/uv/install.sh | sh`.
