...
Usage:
    uv run dataforseo_merchant.py search <keyword> [--marketplace google|amazon] [--location 2840]
    uv run dataforseo_merchant.py sellers <keyword> [--location 2840]
    uv run dataforseo_merchant.py compare <keyword> [--location 2840]
...
try:
    ...
    print(
        json.dumps({"error": "requests library required. Run with: uv run dataforseo_merchant.py (deps auto-install)"}),
        file=sys.stdout,
    )
    sys.exit(1)
